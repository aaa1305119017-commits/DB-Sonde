...
impl ConnectionInner {
    ...
    async fn send_marker(&mut self, marker_type: u8) -> Result<()> {
        ...
        // Header. The length field follows the format negotiated at ACCEPT: 32-bit
        // in large-SDU mode, 16-bit + checksum otherwise. Writing 16-bit on a
        // large-SDU connection makes the server read the length as 0x000b0000 and
        // wait for a packet that never arrives, so it drops the connection.
        if self.large_sdu {
...
impl Connection {
    ...
    /// Whether this handle can still carry statements.
    ///
    /// `is_closed` only reports an explicit `close()` or a drop. A connection the
    /// driver gave up on internally — the server hung up, a marker exchange went
    /// wrong — has `state == Closed` instead, and from then on every operation is
    /// refused at `ensure_ready` with `ConnectionNotReady`. Nothing in the crate
    /// revives it, so a caller holding a long-lived handle needs to be able to see
    /// that and reconnect. Reads local state only; no round trip.
    pub async fn is_usable(&self) -> bool {
        !self.is_closed() && self.inner.lock().await.state == ConnectionState::Ready
    }
    ...
    async fn send_marker(&self, inner: &mut ConnectionInner, marker_type: u8) -> Result<()> {
        ...
        // Length field must follow the negotiated SDU format — see send_marker above.
        if inner.large_sdu {
