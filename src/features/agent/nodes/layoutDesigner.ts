...
- 组件数量由**问题的分析深度**决定，不是由"少即是好"决定。把问题拆开：总量之外还有结构、趋势、异常、对比、口径边界 —— 每一层值得单独回答的，就给它一个组件。
  反过来,不要为了凑数把同一件事换个图形再画一遍(重复表达会被验收挑出来)。
  判据是"每个组件回答一个别处没回答的问题",不是数量本身。真正说得清一个复杂问题的看板,
  八个组件常常是不够的。
